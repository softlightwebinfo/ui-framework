...
import {SoftFormHelpText} from '../form_help_text';
import {SoftFormErrorText} from '../form_error_text';
import {SoftFormLabel} from '../form_label';
import {SoftFlexGroup, SoftFlexItem} from '../../Flex';

import makeId from './make_id';

export class SoftFormRow extends Component<{
  id?: any;
  helpText?: any,
  isInvalid?: any,
  error?: any,
  ...
  render() {
    ...
    const classes = classNames(
      'softFormRow',
      {
        'softFormRow--hasEmptyLabelSpace': hasEmptyLabelSpace,
        'softFormRow--fullWidth': fullWidth,
        'softFormRow--compressed': compressed,
      },
      className
    );
    ...
    if (helpText) {
      optionalHelpText = (
        <SoftFormHelpText id={`${id}-help`} className="softFormRow__text">
          {helpText}
        </SoftFormHelpText>
      );
    }
    ...
    if (error && isInvalid) {
      ...
      optionalErrors = errorTexts.map((error, i) => {
          ...
          return (
            <SoftFormErrorText key={key} id={`${id}-error-${i}`} className="softFormRow__text">
              {error}
            </SoftFormErrorText>
          );
        }
    ...
    if (labelAppend) {
      optionalLabel = (
        <SoftFlexGroup responsive={false} wrap={true} gutterSize="xs" justifyContent="spaceBetween">
          <SoftFlexItem grow={false}>{optionalLabel}</SoftFlexItem>
          <SoftFlexItem grow={false}>{labelAppend}</SoftFlexItem>
        </SoftFlexGroup>
      );
    }
    ...
    if (displayOnly) {
      // @ts-ignore
      field = (
        <div className="softFormRow__displayOnlyWrapper">
          {field}
        </div>
      );
    }
