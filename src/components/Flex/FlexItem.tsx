...
export interface SoftFlexItemProps {
...
export class SoftFlexItem extends PureComponent<{ style?: object, children: any, className?: any, grow?: boolean, component?: string }> {
    ...
    render() {
        ...
        const classes = classNames(
            'softFlexItem',
            {
                'softFlexItem--flexGrowZero': !grow,
                [`softFlexItem--flexGrow${grow}`]:
...
function validateGrowValue(value: SoftFlexItemProps['grow']) {
    const validValues = [null, undefined, true, false, ...GROW_SIZES];

    if (validValues.indexOf(value) === -1) {
        throw new Error(
            `Prop \`grow\` passed to \`SoftFlexItem\` must be a boolean or an integer between 1 and 10, received \`${value}\``
        );
    }
}
